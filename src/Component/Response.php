...
class Response
{
    ...
    public function setCode($code)
    {
        ...
        if (!is_null($this->exception)) throw new $this->exception(Core::getInstance());
    ...
    public function __destruct()
    {
        ...
        if ($core->response->code == 200 && $core->response->exception == null) {
            $this->setController($core->route->routes[$core->request->method][$this->execute]);
            if (is_string($core->response->controller)): $core->response->runControllerFromString();
                return; endif;

            if (is_callable($core->response->controller)) : $core->response->runControllerFromClosure();
                return; endif;

            if (is_array($core->response->controller)) : $core->response->runControllerFromArray();
                return; endif;
        }
